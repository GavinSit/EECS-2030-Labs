...
public class LabTest {
	...
	public void test06_badUnits() {
		final String[] U = { "C", "F", "c", "f", "celcius", "fahrenheit" };
		final boolean[] EXP = { false, false, true, true, true, true };
	...
	public void test08_isInRange() {
		final double[] X = { 100.0, 100.0, -200.5, -200.5 };
		final double[] LO = { 0.0, 99.99, -300, -200.50001 };
		final double[] HI = { 200.0, 100.01, 10.0, -200.4999 };
		for (int i = 0; i < X.length; i++) {
			double x = X[i];
			double lo = LO[i];
			double hi = HI[i];
	...
	public void test09_isInRange() {
		final double[] X = { 0.0, 0.0, 0.0, 0.0 };
		final double[] LO = { 0.0, -10.0, 1.0, -10.0 };
		final double[] HI = { 1.0, 0.0, -1.0, -5.0 };
		for (int i = 0; i < X.length; i++) {
			double x = X[i];
			double lo = LO[i];
			double hi = HI[i];
	...
	public void test13_swapFirstAndLast() {
		...
		assertEquals("swapFirstAndLast(<empty list>) failed", exp, t);
	}
	...
	public void test19_numShortWords() {
		...
		int exp = 5;
		assertEquals(error, exp, Lab.numShortWords(t));
		assertEquals("numShortWords(t) changed the list t", u, t);
	}
